...
static size_t find_part_mathes(const regex_t* regexes, size_t count_regex, t_s21_option options, const char* filename, const char* line, int line_num)
{
    ...
    while (pos)
    {
        ...
        for (size_t i = 0; i < count_regex; ++i) {
            ...
            if (regexec(&regexes[i], pos, 1, &pmatch, 0) == 0)
            {
                ...
                len_min_pos = (curr_pos < min_pos) ? (size_t)(pmatch.rm_eo - pmatch.rm_so) : len_min_pos;
...
static size_t find_mathes(const regex_t* regexes, size_t count_regex, t_s21_option options, const char* filename, const char* line, int line_num)
{
    size_t match_count = 0;

    if (!exists_option(options, OPTION_ONLY_MATCHING)) {
        bool matched = false;
        for (size_t i = 0; i < count_regex && match_count == 0; ++i) {
        ...
        match_count = find_part_mathes(regexes, count_regex, options, filename, line, line_num);
    }
    return match_count;
}

static void handle_file(const regex_t* regex, size_t count_regex, t_s21_option options, const char* filename)
{
    ...
    if (file)
    {
        ...
        while (getline(&line, &len, file) != -1) {
            match_count += (find_mathes(regex, count_regex, options, filename, line, line_num) > 0) ? 1 : 0;
            line_num++;
        }
